...
\title{Systematic Specification for Scalable Verification of Fault-Tolerant Algorithms}
\author{Ben and Lee}
...
An example of a verification optimization is to model message passing as shared state between processes to reduce the number of state variables in the model, since the message passing mechanism is elided. While these optimizations might be safe under particular assumptions, they can lead to unsound models when a system or its environment is modified.
...
\noindent
such that $S_{M | M'} = S \cup S'$,
$$I_{M | M'} = \{ s_I \cup s_{I'} | s_I \in I\textnormal{, }s_{I'} \in I'\textnormal{, and } s_I \textnormal{ and } s_{I'}\textnormal{ are consistent}\}$$
\noindent
and for $s, s' \in S_{M | M'}$, $s \rightarrow_{M | M'} s'$ iff
\begin{itemize}
...
There are two practical benefits of the synchronous observer approach. First, the system $\mathcal{S}$ contains no extraneous modeling context about the environment, assumptions, or requirements, making it easier to decompose $\mathcal{S}$ and to synthesize an implementation directly from it. Second, as described by Rushby~\cite{}, specifying requirements is simpler.
...
\begin{figure}
  \centering
  \includegraphics[width=8cm]{figs/proof-structure-validity}
  \caption{Proof structure for the validity property.}
  \label{fig:compare}
\end{figure}

\begin{figure}
  \centering
  \includegraphics[width=8cm]{figs/proof-structure-agreement}
  \caption{Proof structure for the agreement property.}
  \label{fig:compare}
\end{figure}
