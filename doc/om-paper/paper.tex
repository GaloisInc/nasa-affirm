...
\subsection{Proof by Abstraction}\label{sec:abstract}\label{sec:asms}
...
We have implemented $\OMH(1)$ (as well as many variants, described in Section~\ref{sec:modular}) in the Symbolic Analysis Laboratory~\cite{SAL} (SAL) language. The verification is also carried out in SAL, which contains a suite of model-checkers. In our work, we use infinite-state (SMT-based) $k$-induction~\cite{Moura_boundedmodel,Armando-et-al}.

We have chosen to follow Rushby~\cite{Rushby:OM1} in ``unrolling'' the communication among lieutenants into two sets of logical nodes: relays and receivers. A special node called the source takes the place of the general above. Thus, for example, in the case where 4 nodes execute $\OMH(1)$, our model consists of one source, three relays, and three receivers. We model communication through one-way, typed channels. The source broadcasts a message to each relay which, in turn, each broadcast their messages to all receivers.
